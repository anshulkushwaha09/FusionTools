...
const callGemini = async (apiKey, model, messages) => {
    // Convert messages to Gemini format (simple approximation)
    // Gemini expects: { contents: [{ role: 'user', parts: [{ text: '...' }] }] }
    // Note: 'system' role is not always supported directly in 'contents' for standard Gemini API without beta/system instructions, 
    // so we merge system prompt into first user message or use logic.
    ...
    const data = await response.json();
    // Map response back to OpenAI format for frontend consistency
    return {
        choices: [{
            message: {
...
const callHuggingFace = async (apiKey, model, messages) => {
    // HF Inference API usually takes a prompt string or conversation
    // We'll use the conversational task or text-generation
    ...
    const data = await response.json();
    // HF returns array or object depending on model. Assuming text-generation array.
